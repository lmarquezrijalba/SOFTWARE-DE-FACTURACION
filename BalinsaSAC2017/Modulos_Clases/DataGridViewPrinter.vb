...
Public Class DataGridViewPrinter
    ...
    Shared CurrentRow As Integer ' A static parameter that keep track on which Row (in the DataGridView control) that should be printed
    ...
    Public Sub New(aDataGridView As DataGridView, aPrintDocument As PrintDocument, CenterOnPage As Boolean, WithTitle As Boolean, aTitleText As String, aTitleFont As Font, aTitleColor As Color, WithPaging As Boolean)
    ...
    ' La función que calcula la altura de cada fila (incluyendo la fila de encabezado ),
    ' el ancho de cada columna (de acuerdo con el texto más largo en todas sus células, 
    ' incluyendo la celda de encabezado ) , y toda la anchura DataGridView
    Private Sub Calculate(g As Graphics)
        If PageNumber = 0 Then
            Dim tmpSize As New SizeF() ' Sólo una vez que el cálculo
            Dim tmpFont As Font
            ...
            For i As Integer = 0 To TheDataGridView.Columns.Count - 1
                ...
                End If
                tmpSize = g.MeasureString(TheDataGridView.Columns(i).HeaderText, tmpFont) ' Si no hay un estilo especial HeaderFont , a continuación, utilizar el estilo de fuente por defecto DataGridView
                ...
                For j As Integer = 0 To TheDataGridView.Rows.Count - 1
                    ...
                    If tmpFont Is Nothing Then
                        tmpFont = TheDataGridView.DefaultCellStyle.Font
                    End If
                    tmpSize = g.MeasureString("Anything", tmpFont) ' Si el no hay un estilo de fuente especial de la CurrentRow , a continuación, utilizar la opción por defecto asociado con el control DataGridView
            ...
            ' Definir el inicio / parada de los puntos de columna basado en el ancho de página y el ancho DataGridView
            ' Vamos a usar esto para determinar las columnas que se señalan en cada página y cómo se manejará el envoltorio
            ' Por defecto , la envoltura se occurre tal que el número máximo de columnas para una página será determinar
        End If
        Dim k As Integer
        ...
        ' Sólo nos preocupamos por la manipulación en el que el ancho total DataGridView es más grande que el área de impresión
        If TheDataGridViewWidth > mTempPrintArea Then
            mTempWidth = 0.0F
            For k = 0 To TheDataGridView.Columns.Count - 1
                If TheDataGridView.Columns(k).Visible Then
                    mTempWidth += ColumnsWidth(k)
                    'Si la anchura es más grande que el área de la página , a continuación, definir un nuevo rango de impresión columna
    ...
    Private Sub DrawHeader(g As Graphics)
        ...
        'El cálculo de la coordenada x a partir de que el proceso de impresión se iniciará desde
        ...
        ' Cálculo y dibujo de la Cabecera de límites        
